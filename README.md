...
离线模式不启动后端，因此“同步法宝”按钮会保持禁用；节点筛选、悬停预览、中心聚焦、来源查看、缩放、深浅色切换和布局保存等功能仍可正常使用。直接关联节点采用多层环形布局，其他节点停靠在外围，便于先预览再选择。
...
- 当前离线包包含 56 条经关键词复核的北大法宝真实裁判文书，以及 11 条权威公开典型案例；
- 案例保留案号、案由、法院、裁判日期、摘要、风险判定信号和原文链接；
- 同步结果按“案由命中优先、正文关键词命中兜底”的规则二次清洗，弱相关语义结果不会进入离线图谱；
